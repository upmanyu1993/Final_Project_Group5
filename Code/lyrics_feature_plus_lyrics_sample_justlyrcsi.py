...
def dataset_modelling(df, train_size, val_size):
  ...
  # Dimensionality reduction to 100 coordinates -> optimum point
    ...
    y = df['track_popularity_x'].to_numpy()
    df.drop(['track_popularity_x','track_popularity_y'], axis = 1, inplace=True)
    coors = df[df.columns[3:]].dropna().values
...
sentence_embedding = pd.read_csv('sentence_embedd.csv')
lyrics_feature = pd.read_csv('lyrics_feature.csv')

df = dataset(final_data, sentence_embedding, lyrics_feature)
x_train, y_train, x_test, y_test, x_val, y_val = dataset_modelling(sentence_embedding, 0.8, 0.1)
