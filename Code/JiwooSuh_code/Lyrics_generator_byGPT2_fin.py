...
df.to_csv('artist10df.csv')
print(df.info())
# df = data[['track_id', 'track_name', 'track_artist', 'lyrics.1', 'language', 'track_popularity']]
# df = df[df['language'] == 'en']
# df.drop(['language'], axis=1, inplace=True)
# df.dropna(subset=['lyrics.1'], inplace=True)
# df.to_csv('artist10df.csv')
print(df.info())
...
# artist_lyrics_count = df.groupby('track_artist')['lyrics.1'].count().reset_index()
...
#%%
# def create_artist_lyrics_csv(artist_name, df, output_csv):
#
...
#     create_artist_lyrics_csv(artist, df, output_csv)
#%%
from sklearn.model_selection import train_test_split
def create_train_val_datasets(artist_name, df):
    artist_df = df[df['track_artist'] == artist_name]['lyrics']
    artist_df.reset_index(drop=True, inplace=True)

    # Split the data for each artist
    train_artist_df, eval_artist_df = train_test_split(artist_df, test_size=0.1, random_state=42, shuffle=False)

    # Save the split datasets to CSV files
    train_csv = f'train_{artist_name.lower().replace(" ", "_")}_dataset.csv'
    eval_csv = f'eval_{artist_name.lower().replace(" ", "_")}_dataset.csv'

    train_artist_df.to_csv(train_csv, header=None, index=False)
    eval_artist_df.to_csv(eval_csv, header=None, index=False)

    print(f"Created train dataset for {artist_name} in {train_csv}")
    print(f"Created validation dataset for {artist_name} in {eval_csv}")
    return train_csv, eval_csv

# Iterate through each artist
for artist in artist_lists:
    create_train_val_datasets(artist, df)
...
#     df.dropna(subset=['lyrics.1'], inplace=True)
#
#     artist_df = df[df['track_artist'] == artist_name]['lyrics.1']
#     artist_df.reset_index(drop=True, inplace=True)
#     print(artist_df.head())
#
#     artist_df.to_csv(output_csv, header=None, index=False)
...
def train(train_file_path, eval_file_path, model_name,
    ...
    val_dataset = load_dataset(eval_file_path, tokenizer)
...
def train_all_models():
    for artist in artist_lists:
        ...
        output_dir = f"results2_{artist.lower().replace(' ', '_')}"
        train_file_path, eval_file_path = create_train_val_datasets(artist, df)
        ...
        train(train_file_path, eval_file_path, 'gpt2', output_dir, overwrite_output_dir,
                    per_device_train_batch_size, num_train_epochs, weight_decay, save_steps)
